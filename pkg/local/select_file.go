...
func SelectFile(allowedExt []string, title string) (string, error) {
	...
	expanded := expandHomePath(selectedFile)
...
func (s *FileSelectSession) UpdateOnFilter() {
	...
	isAbsoluteOrHome := strings.HasPrefix(s.Filter, "/") || strings.HasPrefix(s.Filter, "~")
	...
	// 2️⃣ External path (absolute or ~)
	s.IsExternal = true
	s.Hint = "navigate to file"

	expanded := expandHomePath(s.Filter)
...
// expandHomePath expands "~" → user home directory.
...
func collectAllFiles(root string, allowedExt []string) ([]string, error) {
	...
	for _, f := range files {
		trimmed = append(trimmed, strings.TrimPrefix(f, root+"/"))
	}
	return trimmed, nil
}
