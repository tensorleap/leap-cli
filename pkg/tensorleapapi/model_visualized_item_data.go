...
type VisualizedItemData struct {
	CompositeVizData *CompositeVizData
...
func (dst *VisualizedItemData) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into CompositeVizData
	err = json.Unmarshal(data, &dst.CompositeVizData)
	if err == nil {
		jsonCompositeVizData, _ := json.Marshal(dst.CompositeVizData)
		if string(jsonCompositeVizData) == "{}" { // empty struct
			dst.CompositeVizData = nil
		} else {
			return nil // data stored in dst.CompositeVizData, return on the first match
		}
	} else {
		dst.CompositeVizData = nil
	}
...
func (src VisualizedItemData) MarshalJSON() ([]byte, error) {
	if src.CompositeVizData != nil {
		return json.Marshal(&src.CompositeVizData)
	}
