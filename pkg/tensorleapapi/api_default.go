...
type ApiKeyLoginRequest struct {
	ctx context.Context
	ApiService *DefaultApiService
	keyLoginParams *KeyLoginParams
}

func (r ApiKeyLoginRequest) KeyLoginParams(keyLoginParams KeyLoginParams) ApiKeyLoginRequest {
	r.keyLoginParams = &keyLoginParams
	return r
}

func (r ApiKeyLoginRequest) Execute() (*UserData, *http.Response, error) {
	return r.ApiService.KeyLoginExecute(r)
}

/*
KeyLogin Method for KeyLogin

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiKeyLoginRequest
*/
func (a *DefaultApiService) KeyLogin(ctx context.Context) ApiKeyLoginRequest {
	return ApiKeyLoginRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
//  @return UserData
func (a *DefaultApiService) KeyLoginExecute(r ApiKeyLoginRequest) (*UserData, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DefaultApiService.KeyLogin")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/auth/keylogin"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.keyLoginParams == nil {
		return localVarReturnValue, nil, reportError("keyLoginParams is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.keyLoginParams
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
