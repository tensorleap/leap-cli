/*
node-server

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 11.0.133
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tensorleapapi

import (
	"encoding/json"
	"fmt"
	"time"
)

// checks if the DatasetSplitting type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DatasetSplitting{}

// DatasetSplitting struct for DatasetSplitting
type DatasetSplitting struct {
	Id                   string        `json:"id"`
	JobId                string        `json:"jobId"`
	VersionId            string        `json:"versionId"`
	VersionName          string        `json:"versionName"`
	CreatedAt            time.Time     `json:"createdAt"`
	CreatedBy            string        `json:"createdBy"`
	FileUrl              *string       `json:"fileUrl,omitempty"`
	FilterFileUrl        *string       `json:"filterFileUrl,omitempty"`
	Status               JobStatus     `json:"status"`
	IsDeleted            bool          `json:"isDeleted"`
	SplitsToResplit      []SplitSubset `json:"splitsToResplit"`
	KeepTogetherMetadata []string      `json:"keepTogetherMetadata"`
	SplitAcrossMetadata  []string      `json:"splitAcrossMetadata"`
	RunProcess           *RunProcess   `json:"runProcess,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _DatasetSplitting DatasetSplitting

// NewDatasetSplitting instantiates a new DatasetSplitting object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDatasetSplitting(id string, jobId string, versionId string, versionName string, createdAt time.Time, createdBy string, status JobStatus, isDeleted bool, splitsToResplit []SplitSubset, keepTogetherMetadata []string, splitAcrossMetadata []string) *DatasetSplitting {
	this := DatasetSplitting{}
	this.Id = id
	this.JobId = jobId
	this.VersionId = versionId
	this.VersionName = versionName
	this.CreatedAt = createdAt
	this.CreatedBy = createdBy
	this.Status = status
	this.IsDeleted = isDeleted
	this.SplitsToResplit = splitsToResplit
	this.KeepTogetherMetadata = keepTogetherMetadata
	this.SplitAcrossMetadata = splitAcrossMetadata
	return &this
}

// NewDatasetSplittingWithDefaults instantiates a new DatasetSplitting object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDatasetSplittingWithDefaults() *DatasetSplitting {
	this := DatasetSplitting{}
	return &this
}

// GetId returns the Id field value
func (o *DatasetSplitting) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *DatasetSplitting) SetId(v string) {
	o.Id = v
}

// GetJobId returns the JobId field value
func (o *DatasetSplitting) GetJobId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.JobId
}

// GetJobIdOk returns a tuple with the JobId field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetJobIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.JobId, true
}

// SetJobId sets field value
func (o *DatasetSplitting) SetJobId(v string) {
	o.JobId = v
}

// GetVersionId returns the VersionId field value
func (o *DatasetSplitting) GetVersionId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.VersionId
}

// GetVersionIdOk returns a tuple with the VersionId field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetVersionIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VersionId, true
}

// SetVersionId sets field value
func (o *DatasetSplitting) SetVersionId(v string) {
	o.VersionId = v
}

// GetVersionName returns the VersionName field value
func (o *DatasetSplitting) GetVersionName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.VersionName
}

// GetVersionNameOk returns a tuple with the VersionName field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetVersionNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VersionName, true
}

// SetVersionName sets field value
func (o *DatasetSplitting) SetVersionName(v string) {
	o.VersionName = v
}

// GetCreatedAt returns the CreatedAt field value
func (o *DatasetSplitting) GetCreatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedAt, true
}

// SetCreatedAt sets field value
func (o *DatasetSplitting) SetCreatedAt(v time.Time) {
	o.CreatedAt = v
}

// GetCreatedBy returns the CreatedBy field value
func (o *DatasetSplitting) GetCreatedBy() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.CreatedBy
}

// GetCreatedByOk returns a tuple with the CreatedBy field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetCreatedByOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedBy, true
}

// SetCreatedBy sets field value
func (o *DatasetSplitting) SetCreatedBy(v string) {
	o.CreatedBy = v
}

// GetFileUrl returns the FileUrl field value if set, zero value otherwise.
func (o *DatasetSplitting) GetFileUrl() string {
	if o == nil || IsNil(o.FileUrl) {
		var ret string
		return ret
	}
	return *o.FileUrl
}

// GetFileUrlOk returns a tuple with the FileUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetFileUrlOk() (*string, bool) {
	if o == nil || IsNil(o.FileUrl) {
		return nil, false
	}
	return o.FileUrl, true
}

// HasFileUrl returns a boolean if a field has been set.
func (o *DatasetSplitting) HasFileUrl() bool {
	if o != nil && !IsNil(o.FileUrl) {
		return true
	}

	return false
}

// SetFileUrl gets a reference to the given string and assigns it to the FileUrl field.
func (o *DatasetSplitting) SetFileUrl(v string) {
	o.FileUrl = &v
}

// GetFilterFileUrl returns the FilterFileUrl field value if set, zero value otherwise.
func (o *DatasetSplitting) GetFilterFileUrl() string {
	if o == nil || IsNil(o.FilterFileUrl) {
		var ret string
		return ret
	}
	return *o.FilterFileUrl
}

// GetFilterFileUrlOk returns a tuple with the FilterFileUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetFilterFileUrlOk() (*string, bool) {
	if o == nil || IsNil(o.FilterFileUrl) {
		return nil, false
	}
	return o.FilterFileUrl, true
}

// HasFilterFileUrl returns a boolean if a field has been set.
func (o *DatasetSplitting) HasFilterFileUrl() bool {
	if o != nil && !IsNil(o.FilterFileUrl) {
		return true
	}

	return false
}

// SetFilterFileUrl gets a reference to the given string and assigns it to the FilterFileUrl field.
func (o *DatasetSplitting) SetFilterFileUrl(v string) {
	o.FilterFileUrl = &v
}

// GetStatus returns the Status field value
func (o *DatasetSplitting) GetStatus() JobStatus {
	if o == nil {
		var ret JobStatus
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetStatusOk() (*JobStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *DatasetSplitting) SetStatus(v JobStatus) {
	o.Status = v
}

// GetIsDeleted returns the IsDeleted field value
func (o *DatasetSplitting) GetIsDeleted() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsDeleted
}

// GetIsDeletedOk returns a tuple with the IsDeleted field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetIsDeletedOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsDeleted, true
}

// SetIsDeleted sets field value
func (o *DatasetSplitting) SetIsDeleted(v bool) {
	o.IsDeleted = v
}

// GetSplitsToResplit returns the SplitsToResplit field value
func (o *DatasetSplitting) GetSplitsToResplit() []SplitSubset {
	if o == nil {
		var ret []SplitSubset
		return ret
	}

	return o.SplitsToResplit
}

// GetSplitsToResplitOk returns a tuple with the SplitsToResplit field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetSplitsToResplitOk() ([]SplitSubset, bool) {
	if o == nil {
		return nil, false
	}
	return o.SplitsToResplit, true
}

// SetSplitsToResplit sets field value
func (o *DatasetSplitting) SetSplitsToResplit(v []SplitSubset) {
	o.SplitsToResplit = v
}

// GetKeepTogetherMetadata returns the KeepTogetherMetadata field value
func (o *DatasetSplitting) GetKeepTogetherMetadata() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.KeepTogetherMetadata
}

// GetKeepTogetherMetadataOk returns a tuple with the KeepTogetherMetadata field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetKeepTogetherMetadataOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.KeepTogetherMetadata, true
}

// SetKeepTogetherMetadata sets field value
func (o *DatasetSplitting) SetKeepTogetherMetadata(v []string) {
	o.KeepTogetherMetadata = v
}

// GetSplitAcrossMetadata returns the SplitAcrossMetadata field value
func (o *DatasetSplitting) GetSplitAcrossMetadata() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.SplitAcrossMetadata
}

// GetSplitAcrossMetadataOk returns a tuple with the SplitAcrossMetadata field value
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetSplitAcrossMetadataOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SplitAcrossMetadata, true
}

// SetSplitAcrossMetadata sets field value
func (o *DatasetSplitting) SetSplitAcrossMetadata(v []string) {
	o.SplitAcrossMetadata = v
}

// GetRunProcess returns the RunProcess field value if set, zero value otherwise.
func (o *DatasetSplitting) GetRunProcess() RunProcess {
	if o == nil || IsNil(o.RunProcess) {
		var ret RunProcess
		return ret
	}
	return *o.RunProcess
}

// GetRunProcessOk returns a tuple with the RunProcess field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DatasetSplitting) GetRunProcessOk() (*RunProcess, bool) {
	if o == nil || IsNil(o.RunProcess) {
		return nil, false
	}
	return o.RunProcess, true
}

// HasRunProcess returns a boolean if a field has been set.
func (o *DatasetSplitting) HasRunProcess() bool {
	if o != nil && !IsNil(o.RunProcess) {
		return true
	}

	return false
}

// SetRunProcess gets a reference to the given RunProcess and assigns it to the RunProcess field.
func (o *DatasetSplitting) SetRunProcess(v RunProcess) {
	o.RunProcess = &v
}

func (o DatasetSplitting) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DatasetSplitting) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["jobId"] = o.JobId
	toSerialize["versionId"] = o.VersionId
	toSerialize["versionName"] = o.VersionName
	toSerialize["createdAt"] = o.CreatedAt
	toSerialize["createdBy"] = o.CreatedBy
	if !IsNil(o.FileUrl) {
		toSerialize["fileUrl"] = o.FileUrl
	}
	if !IsNil(o.FilterFileUrl) {
		toSerialize["filterFileUrl"] = o.FilterFileUrl
	}
	toSerialize["status"] = o.Status
	toSerialize["isDeleted"] = o.IsDeleted
	toSerialize["splitsToResplit"] = o.SplitsToResplit
	toSerialize["keepTogetherMetadata"] = o.KeepTogetherMetadata
	toSerialize["splitAcrossMetadata"] = o.SplitAcrossMetadata
	if !IsNil(o.RunProcess) {
		toSerialize["runProcess"] = o.RunProcess
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *DatasetSplitting) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"jobId",
		"versionId",
		"versionName",
		"createdAt",
		"createdBy",
		"status",
		"isDeleted",
		"splitsToResplit",
		"keepTogetherMetadata",
		"splitAcrossMetadata",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varDatasetSplitting := _DatasetSplitting{}

	err = json.Unmarshal(data, &varDatasetSplitting)

	if err != nil {
		return err
	}

	*o = DatasetSplitting(varDatasetSplitting)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "jobId")
		delete(additionalProperties, "versionId")
		delete(additionalProperties, "versionName")
		delete(additionalProperties, "createdAt")
		delete(additionalProperties, "createdBy")
		delete(additionalProperties, "fileUrl")
		delete(additionalProperties, "filterFileUrl")
		delete(additionalProperties, "status")
		delete(additionalProperties, "isDeleted")
		delete(additionalProperties, "splitsToResplit")
		delete(additionalProperties, "keepTogetherMetadata")
		delete(additionalProperties, "splitAcrossMetadata")
		delete(additionalProperties, "runProcess")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableDatasetSplitting struct {
	value *DatasetSplitting
	isSet bool
}

func (v NullableDatasetSplitting) Get() *DatasetSplitting {
	return v.value
}

func (v *NullableDatasetSplitting) Set(val *DatasetSplitting) {
	v.value = val
	v.isSet = true
}

func (v NullableDatasetSplitting) IsSet() bool {
	return v.isSet
}

func (v *NullableDatasetSplitting) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDatasetSplitting(val *DatasetSplitting) *NullableDatasetSplitting {
	return &NullableDatasetSplitting{value: val, isSet: true}
}

func (v NullableDatasetSplitting) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDatasetSplitting) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
