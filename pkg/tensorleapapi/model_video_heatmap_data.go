/*
node-server

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 11.0.20-rc.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tensorleapapi

import (
	"encoding/json"
	"fmt"
)

// checks if the VideoHeatmapData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &VideoHeatmapData{}

// VideoHeatmapData struct for VideoHeatmapData
type VideoHeatmapData struct {
	VideoBlob            string       `json:"video_blob"`
	HeatmapBlob          string       `json:"heatmap_blob"`
	Type                 DataTypeEnum `json:"type"`
	AdditionalProperties map[string]interface{}
}

type _VideoHeatmapData VideoHeatmapData

// NewVideoHeatmapData instantiates a new VideoHeatmapData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVideoHeatmapData(videoBlob string, heatmapBlob string, type_ DataTypeEnum) *VideoHeatmapData {
	this := VideoHeatmapData{}
	this.VideoBlob = videoBlob
	this.HeatmapBlob = heatmapBlob
	this.Type = type_
	return &this
}

// NewVideoHeatmapDataWithDefaults instantiates a new VideoHeatmapData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVideoHeatmapDataWithDefaults() *VideoHeatmapData {
	this := VideoHeatmapData{}
	return &this
}

// GetVideoBlob returns the VideoBlob field value
func (o *VideoHeatmapData) GetVideoBlob() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.VideoBlob
}

// GetVideoBlobOk returns a tuple with the VideoBlob field value
// and a boolean to check if the value has been set.
func (o *VideoHeatmapData) GetVideoBlobOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VideoBlob, true
}

// SetVideoBlob sets field value
func (o *VideoHeatmapData) SetVideoBlob(v string) {
	o.VideoBlob = v
}

// GetHeatmapBlob returns the HeatmapBlob field value
func (o *VideoHeatmapData) GetHeatmapBlob() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.HeatmapBlob
}

// GetHeatmapBlobOk returns a tuple with the HeatmapBlob field value
// and a boolean to check if the value has been set.
func (o *VideoHeatmapData) GetHeatmapBlobOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.HeatmapBlob, true
}

// SetHeatmapBlob sets field value
func (o *VideoHeatmapData) SetHeatmapBlob(v string) {
	o.HeatmapBlob = v
}

// GetType returns the Type field value
func (o *VideoHeatmapData) GetType() DataTypeEnum {
	if o == nil {
		var ret DataTypeEnum
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *VideoHeatmapData) GetTypeOk() (*DataTypeEnum, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *VideoHeatmapData) SetType(v DataTypeEnum) {
	o.Type = v
}

func (o VideoHeatmapData) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o VideoHeatmapData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["video_blob"] = o.VideoBlob
	toSerialize["heatmap_blob"] = o.HeatmapBlob
	toSerialize["type"] = o.Type

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *VideoHeatmapData) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"video_blob",
		"heatmap_blob",
		"type",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varVideoHeatmapData := _VideoHeatmapData{}

	err = json.Unmarshal(data, &varVideoHeatmapData)

	if err != nil {
		return err
	}

	*o = VideoHeatmapData(varVideoHeatmapData)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "video_blob")
		delete(additionalProperties, "heatmap_blob")
		delete(additionalProperties, "type")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableVideoHeatmapData struct {
	value *VideoHeatmapData
	isSet bool
}

func (v NullableVideoHeatmapData) Get() *VideoHeatmapData {
	return v.value
}

func (v *NullableVideoHeatmapData) Set(val *VideoHeatmapData) {
	v.value = val
	v.isSet = true
}

func (v NullableVideoHeatmapData) IsSet() bool {
	return v.isSet
}

func (v *NullableVideoHeatmapData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVideoHeatmapData(val *VideoHeatmapData) *NullableVideoHeatmapData {
	return &NullableVideoHeatmapData{value: val, isSet: true}
}

func (v NullableVideoHeatmapData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVideoHeatmapData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
