/*
node-server

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.0.653-rc.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tensorleapapi

import (
	"encoding/json"
	"fmt"
)

// checks if the EvaluateExistingSessionParams type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &EvaluateExistingSessionParams{}

// EvaluateExistingSessionParams struct for EvaluateExistingSessionParams
type EvaluateExistingSessionParams struct {
	VersionId             string          `json:"versionId"`
	ProjectId             string          `json:"projectId"`
	BatchSize             float64         `json:"batchSize"`
	DataStates            []DataStateType `json:"dataStates"`
	Name                  string          `json:"name"`
	Description           string          `json:"description"`
	SkipMetricsEstimation bool            `json:"skipMetricsEstimation"`
	Monitor               *bool           `json:"monitor,omitempty"`
	SmartVisualizations   *bool           `json:"smartVisualizations,omitempty"`
	EvaluatedEpoch        float64         `json:"evaluatedEpoch"`
	SessionId             string          `json:"sessionId"`
	AdditionalProperties  map[string]interface{}
}

type _EvaluateExistingSessionParams EvaluateExistingSessionParams

// NewEvaluateExistingSessionParams instantiates a new EvaluateExistingSessionParams object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewEvaluateExistingSessionParams(versionId string, projectId string, batchSize float64, dataStates []DataStateType, name string, description string, skipMetricsEstimation bool, evaluatedEpoch float64, sessionId string) *EvaluateExistingSessionParams {
	this := EvaluateExistingSessionParams{}
	this.VersionId = versionId
	this.ProjectId = projectId
	this.BatchSize = batchSize
	this.DataStates = dataStates
	this.Name = name
	this.Description = description
	this.SkipMetricsEstimation = skipMetricsEstimation
	this.EvaluatedEpoch = evaluatedEpoch
	this.SessionId = sessionId
	return &this
}

// NewEvaluateExistingSessionParamsWithDefaults instantiates a new EvaluateExistingSessionParams object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewEvaluateExistingSessionParamsWithDefaults() *EvaluateExistingSessionParams {
	this := EvaluateExistingSessionParams{}
	return &this
}

// GetVersionId returns the VersionId field value
func (o *EvaluateExistingSessionParams) GetVersionId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.VersionId
}

// GetVersionIdOk returns a tuple with the VersionId field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetVersionIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VersionId, true
}

// SetVersionId sets field value
func (o *EvaluateExistingSessionParams) SetVersionId(v string) {
	o.VersionId = v
}

// GetProjectId returns the ProjectId field value
func (o *EvaluateExistingSessionParams) GetProjectId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ProjectId
}

// GetProjectIdOk returns a tuple with the ProjectId field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetProjectIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ProjectId, true
}

// SetProjectId sets field value
func (o *EvaluateExistingSessionParams) SetProjectId(v string) {
	o.ProjectId = v
}

// GetBatchSize returns the BatchSize field value
func (o *EvaluateExistingSessionParams) GetBatchSize() float64 {
	if o == nil {
		var ret float64
		return ret
	}

	return o.BatchSize
}

// GetBatchSizeOk returns a tuple with the BatchSize field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetBatchSizeOk() (*float64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.BatchSize, true
}

// SetBatchSize sets field value
func (o *EvaluateExistingSessionParams) SetBatchSize(v float64) {
	o.BatchSize = v
}

// GetDataStates returns the DataStates field value
func (o *EvaluateExistingSessionParams) GetDataStates() []DataStateType {
	if o == nil {
		var ret []DataStateType
		return ret
	}

	return o.DataStates
}

// GetDataStatesOk returns a tuple with the DataStates field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetDataStatesOk() ([]DataStateType, bool) {
	if o == nil {
		return nil, false
	}
	return o.DataStates, true
}

// SetDataStates sets field value
func (o *EvaluateExistingSessionParams) SetDataStates(v []DataStateType) {
	o.DataStates = v
}

// GetName returns the Name field value
func (o *EvaluateExistingSessionParams) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *EvaluateExistingSessionParams) SetName(v string) {
	o.Name = v
}

// GetDescription returns the Description field value
func (o *EvaluateExistingSessionParams) GetDescription() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Description
}

// GetDescriptionOk returns a tuple with the Description field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Description, true
}

// SetDescription sets field value
func (o *EvaluateExistingSessionParams) SetDescription(v string) {
	o.Description = v
}

// GetSkipMetricsEstimation returns the SkipMetricsEstimation field value
func (o *EvaluateExistingSessionParams) GetSkipMetricsEstimation() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.SkipMetricsEstimation
}

// GetSkipMetricsEstimationOk returns a tuple with the SkipMetricsEstimation field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetSkipMetricsEstimationOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SkipMetricsEstimation, true
}

// SetSkipMetricsEstimation sets field value
func (o *EvaluateExistingSessionParams) SetSkipMetricsEstimation(v bool) {
	o.SkipMetricsEstimation = v
}

// GetMonitor returns the Monitor field value if set, zero value otherwise.
func (o *EvaluateExistingSessionParams) GetMonitor() bool {
	if o == nil || IsNil(o.Monitor) {
		var ret bool
		return ret
	}
	return *o.Monitor
}

// GetMonitorOk returns a tuple with the Monitor field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetMonitorOk() (*bool, bool) {
	if o == nil || IsNil(o.Monitor) {
		return nil, false
	}
	return o.Monitor, true
}

// HasMonitor returns a boolean if a field has been set.
func (o *EvaluateExistingSessionParams) HasMonitor() bool {
	if o != nil && !IsNil(o.Monitor) {
		return true
	}

	return false
}

// SetMonitor gets a reference to the given bool and assigns it to the Monitor field.
func (o *EvaluateExistingSessionParams) SetMonitor(v bool) {
	o.Monitor = &v
}

// GetSmartVisualizations returns the SmartVisualizations field value if set, zero value otherwise.
func (o *EvaluateExistingSessionParams) GetSmartVisualizations() bool {
	if o == nil || IsNil(o.SmartVisualizations) {
		var ret bool
		return ret
	}
	return *o.SmartVisualizations
}

// GetSmartVisualizationsOk returns a tuple with the SmartVisualizations field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetSmartVisualizationsOk() (*bool, bool) {
	if o == nil || IsNil(o.SmartVisualizations) {
		return nil, false
	}
	return o.SmartVisualizations, true
}

// HasSmartVisualizations returns a boolean if a field has been set.
func (o *EvaluateExistingSessionParams) HasSmartVisualizations() bool {
	if o != nil && !IsNil(o.SmartVisualizations) {
		return true
	}

	return false
}

// SetSmartVisualizations gets a reference to the given bool and assigns it to the SmartVisualizations field.
func (o *EvaluateExistingSessionParams) SetSmartVisualizations(v bool) {
	o.SmartVisualizations = &v
}

// GetEvaluatedEpoch returns the EvaluatedEpoch field value
func (o *EvaluateExistingSessionParams) GetEvaluatedEpoch() float64 {
	if o == nil {
		var ret float64
		return ret
	}

	return o.EvaluatedEpoch
}

// GetEvaluatedEpochOk returns a tuple with the EvaluatedEpoch field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetEvaluatedEpochOk() (*float64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.EvaluatedEpoch, true
}

// SetEvaluatedEpoch sets field value
func (o *EvaluateExistingSessionParams) SetEvaluatedEpoch(v float64) {
	o.EvaluatedEpoch = v
}

// GetSessionId returns the SessionId field value
func (o *EvaluateExistingSessionParams) GetSessionId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SessionId
}

// GetSessionIdOk returns a tuple with the SessionId field value
// and a boolean to check if the value has been set.
func (o *EvaluateExistingSessionParams) GetSessionIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SessionId, true
}

// SetSessionId sets field value
func (o *EvaluateExistingSessionParams) SetSessionId(v string) {
	o.SessionId = v
}

func (o EvaluateExistingSessionParams) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o EvaluateExistingSessionParams) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["versionId"] = o.VersionId
	toSerialize["projectId"] = o.ProjectId
	toSerialize["batchSize"] = o.BatchSize
	toSerialize["dataStates"] = o.DataStates
	toSerialize["name"] = o.Name
	toSerialize["description"] = o.Description
	toSerialize["skipMetricsEstimation"] = o.SkipMetricsEstimation
	if !IsNil(o.Monitor) {
		toSerialize["monitor"] = o.Monitor
	}
	if !IsNil(o.SmartVisualizations) {
		toSerialize["smartVisualizations"] = o.SmartVisualizations
	}
	toSerialize["evaluatedEpoch"] = o.EvaluatedEpoch
	toSerialize["sessionId"] = o.SessionId

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *EvaluateExistingSessionParams) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"versionId",
		"projectId",
		"batchSize",
		"dataStates",
		"name",
		"description",
		"skipMetricsEstimation",
		"evaluatedEpoch",
		"sessionId",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varEvaluateExistingSessionParams := _EvaluateExistingSessionParams{}

	err = json.Unmarshal(data, &varEvaluateExistingSessionParams)

	if err != nil {
		return err
	}

	*o = EvaluateExistingSessionParams(varEvaluateExistingSessionParams)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "versionId")
		delete(additionalProperties, "projectId")
		delete(additionalProperties, "batchSize")
		delete(additionalProperties, "dataStates")
		delete(additionalProperties, "name")
		delete(additionalProperties, "description")
		delete(additionalProperties, "skipMetricsEstimation")
		delete(additionalProperties, "monitor")
		delete(additionalProperties, "smartVisualizations")
		delete(additionalProperties, "evaluatedEpoch")
		delete(additionalProperties, "sessionId")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableEvaluateExistingSessionParams struct {
	value *EvaluateExistingSessionParams
	isSet bool
}

func (v NullableEvaluateExistingSessionParams) Get() *EvaluateExistingSessionParams {
	return v.value
}

func (v *NullableEvaluateExistingSessionParams) Set(val *EvaluateExistingSessionParams) {
	v.value = val
	v.isSet = true
}

func (v NullableEvaluateExistingSessionParams) IsSet() bool {
	return v.isSet
}

func (v *NullableEvaluateExistingSessionParams) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableEvaluateExistingSessionParams(val *EvaluateExistingSessionParams) *NullableEvaluateExistingSessionParams {
	return &NullableEvaluateExistingSessionParams{value: val, isSet: true}
}

func (v NullableEvaluateExistingSessionParams) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableEvaluateExistingSessionParams) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
