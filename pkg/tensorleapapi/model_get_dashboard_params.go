/*
node-server

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.0.653-rc.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tensorleapapi

import (
	"encoding/json"
	"fmt"
)

// checks if the GetDashboardParams type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetDashboardParams{}

// GetDashboardParams struct for GetDashboardParams
type GetDashboardParams struct {
	DashboardId          string `json:"dashboardId"`
	ProjectId            string `json:"projectId"`
	AdditionalProperties map[string]interface{}
}

type _GetDashboardParams GetDashboardParams

// NewGetDashboardParams instantiates a new GetDashboardParams object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetDashboardParams(dashboardId string, projectId string) *GetDashboardParams {
	this := GetDashboardParams{}
	this.DashboardId = dashboardId
	this.ProjectId = projectId
	return &this
}

// NewGetDashboardParamsWithDefaults instantiates a new GetDashboardParams object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetDashboardParamsWithDefaults() *GetDashboardParams {
	this := GetDashboardParams{}
	return &this
}

// GetDashboardId returns the DashboardId field value
func (o *GetDashboardParams) GetDashboardId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.DashboardId
}

// GetDashboardIdOk returns a tuple with the DashboardId field value
// and a boolean to check if the value has been set.
func (o *GetDashboardParams) GetDashboardIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DashboardId, true
}

// SetDashboardId sets field value
func (o *GetDashboardParams) SetDashboardId(v string) {
	o.DashboardId = v
}

// GetProjectId returns the ProjectId field value
func (o *GetDashboardParams) GetProjectId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ProjectId
}

// GetProjectIdOk returns a tuple with the ProjectId field value
// and a boolean to check if the value has been set.
func (o *GetDashboardParams) GetProjectIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ProjectId, true
}

// SetProjectId sets field value
func (o *GetDashboardParams) SetProjectId(v string) {
	o.ProjectId = v
}

func (o GetDashboardParams) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetDashboardParams) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["dashboardId"] = o.DashboardId
	toSerialize["projectId"] = o.ProjectId

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *GetDashboardParams) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"dashboardId",
		"projectId",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetDashboardParams := _GetDashboardParams{}

	err = json.Unmarshal(data, &varGetDashboardParams)

	if err != nil {
		return err
	}

	*o = GetDashboardParams(varGetDashboardParams)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "dashboardId")
		delete(additionalProperties, "projectId")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableGetDashboardParams struct {
	value *GetDashboardParams
	isSet bool
}

func (v NullableGetDashboardParams) Get() *GetDashboardParams {
	return v.value
}

func (v *NullableGetDashboardParams) Set(val *GetDashboardParams) {
	v.value = val
	v.isSet = true
}

func (v NullableGetDashboardParams) IsSet() bool {
	return v.isSet
}

func (v *NullableGetDashboardParams) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetDashboardParams(val *GetDashboardParams) *NullableGetDashboardParams {
	return &NullableGetDashboardParams{value: val, isSet: true}
}

func (v NullableGetDashboardParams) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetDashboardParams) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
