## leap server tools kubectl create secret tls
...
```
leap server tools kubectl create secret tls NAME --cert=path/to/cert/file --key=path/to/key/file [--dry-run=server|client|none]
```
