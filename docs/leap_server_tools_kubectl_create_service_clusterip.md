## leap server tools kubectl create service clusterip
...
```
leap server tools kubectl create service clusterip NAME [--tcp=<port>:<targetPort>] [--dry-run=server|client|none]
```
