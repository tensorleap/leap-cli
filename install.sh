...
trackEvent() {
  ...
  if type "curl" > /dev/null && [[ -z "${TESTING:-}" ]]; then
    ...
    local event_data
    event_data="{\"event\":\"$event_type\",\"properties\":{\"token\":\"0c1710c9656bbfb1056bb46093e23ca1\",\"time\":$(date +%s),\"os\":\"$(uname | tr '[:upper:]' '[:lower:]')\",\"arch\":\"$(uname -m)\",\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"\$device_id\":\"$device_id\",\"distinct_id\":\"$device_id\",\"whoami\":\"$current_user\",\"aws_environment\":\"$aws_env\""
